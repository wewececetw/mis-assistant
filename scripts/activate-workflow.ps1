...
$result = Invoke-RestMethod -Uri "http://localhost:5678/api/v1/workflows/$WorkflowId" -Method PUT -Headers $headers -Body $body

Write-Host "Activated: $($result.name) - active=$($result.active)"
